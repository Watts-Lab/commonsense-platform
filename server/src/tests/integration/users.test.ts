...
import { send_magic_link } from '../../controllers/emails';
...
describe('Users Route Integration', () => {
  ...
  it('POST /api/users/enter should register a new user and send signup magic link', async () => {
    ...
    const createdUser = await (db as any).users.findOne({
      where: { email: 'new-user@example.com' },
    });
    expect(createdUser).not.toBeNull();
    expect(createdUser.get('sessionId')).toBe('new-user-session');
    const createdMagicLink = createdUser.get('magicLink') as string;
    expect(typeof createdMagicLink).toBe('string');
    expect(createdMagicLink.length).toBeGreaterThan(20);
    expect(send_magic_link as unknown as jest.Mock).toHaveBeenCalledWith(
  ...
  it('POST /api/users/enter should rotate magic link for existing user without token', async () => {
    const existing = await (db as any).users.create({
    ...
    const updated = await (db as any).users.findByPk(existing.get('id'));
    expect(updated.get('magicLink')).not.toBe('old-link');
    expect(updated.get('magicLinkExpired')).toBe(false);
    expect(send_magic_link as unknown as jest.Mock).toHaveBeenCalledWith(
      'existing-user@example.com',
      updated.get('magicLink'),
    );
  });
  ...
  it('POST /api/users/enter should verify valid magic link and set session for user missing one', async () => {
    ...
    const updated = await (db as any).users.findByPk(user.get('id'));
    expect(updated.get('sessionId')).toBe('fresh-session-id');
    expect(updated.get('magicLinkExpired')).toBe(true);
  });
  ...
  it('POST /api/users/deleteaccount should delete matching user for valid token', async () => {
    ...
    const user = await (db as any).users.findOne({
