...
describe("survey persistence and session robustness", () => {
  ...
  it("should preserve progress and session after refresh", () => {
    ...
    cy.get('h2').first().then(($h2) => {
      const firstStatementText = $h2.text();
      ...
      cy.get('h2').first().should(($h2next) => {
        expect($h2next.text()).to.not.equal(firstStatementText);
      });

      cy.get('h2').first().then(($h2second) => {
        const secondStatementText = $h2second.text();
        ...
        cy.get('h2').first().should('have.text', secondStatementText);
