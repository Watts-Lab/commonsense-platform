...
FROM --platform=linux/amd64 public.ecr.aws/docker/library/node:24 AS client-build
WORKDIR /app/client
...
FROM --platform=linux/amd64 public.ecr.aws/docker/library/node:24 AS server-build
WORKDIR /app/server
...
CMD apt-get update && echo "Y" | apt-get install nodejs && service nginx start && cd /usr/src/app && node dist/server.js
